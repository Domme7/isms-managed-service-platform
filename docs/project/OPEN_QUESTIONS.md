...
| O-WP020-06 | Trust-Layer-Angaben ohne Träger: Modell-/Regelversion, Auswirkung von Datenlücken. **Teilweise belegt** (präzisiert im Slice-5-Bau, von Domain- und Konzepttreue-Gate bestätigt): Annahmen (`effectiveness_assumption` — **CCP-004-vorbehaltlich**, fällt bei Option a auf „kein Träger" zurück), menschliche Reviews (Bestätigungsstufe „reviewed" ohne Person/Zeitpunkt/Ergebnis) | Datenlücke | Abgleich sichtbar auf Objekt-360; Register 2026-07-23 nachgezogen | Concept Author / Seed (WP-021) + CCP-004 |
...
| O-WP020-18 | Biome meldet die Komponenten-Prop `role={null}` fälschlich als ARIA-Attribut (`useValidAriaRole`) — 9 wortgleiche Suppressions in Testdateien. Konfigurationsoption `ignoreNonDom: true` oder Prop umbenennen? | Tooling-Folgeaufgabe (Code-Gate) | Suppressions bleiben eng und dokumentiert; Konfig-Änderung nur als eigener kleiner Tooling-Change mit Review | platform-devops + Code-Gate |
| O-WP020-19 | „Wirkung"-Deckungsgrad des Decision-Card-Abgleichs (`/entscheidungen`) steht seit dem WP-020-Fix-Pass auf **teilweise** statt „kein Träger" – gestützt auf `effectiveness_assumption`, das **nicht PDF-gedeckt** ist (CCP-004 offen) und nur an Service-, nicht Entscheidungs-Beziehungen lebt. Domain-Gate bejaht (Konsistenz zum Trust-Layer), QA/Code sehen es als owner-gate-abhängig | Konzept-/Owner-Gate | user-sichtbar mit CCP-004-Vorbehalt im Trägertext; **fällt CCP-004 auf „Feld entfernen", fällt der Eintrag auf „kein Träger" zurück** (`data.ts` + `data.test.ts` + Zählassertions zurückzudrehen) | Owner (mit CCP-004) |
...
| O-WP032-15 | `/reports` rendert PDF-wörtliche **Typnamen** mit Bewertungsvokabular („Reifegrad- und Zielpfadbericht", „Trend Chart", „Control-Wirksamkeitsbericht") — Namen von Berichten des Zielprodukts, **keine Aussagen über den Mandanten** (Grundlagen-Abschnitt ist per Test davon frei) | Abgrenzung zu DR-0008 | wörtlich belassen; Grundlagen-Abschnitt wertungsfrei getestet | QA + Domain (quittieren) |
...
| O-WP028-08 | `OBJECT_FAMILIES.F01.name` heißt „Tenant & Unternehmenskontext" — der Familienname aus Dok. 07 trägt weiter „Tenant" und rendert seit dem Fix-Pass sichtbar (Badge, Objektkarte, Glossar), während DR-0013 Nr. 9 „Mandant" festlegt | Konzept-/Contract-Frage | **nicht angefasst** — Änderung wäre `packages/contracts` + Konzeptwortlaut (Regel Null) | Concept Author + Owner |
| O-WP028-09 | Reichweite des Wortverbots „ohne Träger": DR-0013 Nr. 5 verortet es beim Rollenfokus; „kein Träger" bleibt auf `/entscheidungen` und Objekt-360 als Ehrlichkeits-Substanz | Auslegungsfrage | Wächter verbietet gezielt „ohne Träger", erlaubt „kein Träger" (Negativbeweis) | QA (falls app-weit gemeint) |
| O-WP028-10 | Verhaltensänderung durch „eine Sphären-Regel": neutraler Einstieg und R12 sehen die Portfolio-Verdichtung auf `/services` jetzt (wie schon auf `/twin`), R07 nicht mehr | bewusste Folge der Vereinheitlichung | im Fix-Pass so umgesetzt und getestet; fachlich vom Domain-Gate zu quittieren | Domain |
| O-WP028-11 | „Tenant" ist app-weit noch an drei sichtbaren Stellen: Objekttyp (erledigt → „Mandant (Tenant)"), **Familienname F01 „Tenant & Unternehmenskontext"** (aus Dok. 07, O-WP028-08) und **Rollenname R12 „Tenant/Platform Administrator"** (aus Dok. 03) — beide letzteren aus dem Konzept, Änderung berührt `packages/contracts`/`roles.ts` + Konzeptwortlaut | Konzept-/Leitbegriff-Frage | die zwei Konzept-Vorkommen bewusst unverändert (Regel Null); mechanische Wächter-Sicherung „kein blankes ‚Tenant' im UI außer als Glosse" möglich | Concept Author + Owner |
| O-WP028-12 | Neutraler Zustand (= Default nach jedem Login, DR-0009) sieht auf `/services` **und** `/twin` die mandantenübergreifende Portfolio-Übersicht mit allen synthetischen Mandantennamen. Konsistent mit dem seit WP-004 bestehenden Portfolio-Explorer, aber der Default zeigt damit „fremde" (synthetische) Namen | Verhaltens-/Sicherheitsfrage (Demo) | Verhalten belassen (konsistent, rein synthetisch), kein Text behauptet Exklusivität/Durchsetzung; endgültige Regel mit echter Auth (WP-030) | Security + Owner |

## WP-006 – Kundenwelt Stufe 1 Slice 2+3 (Servicekatalog + Struktur-Assistent)

Slice 2 (`/services/katalog`) und Slice 3 (`/kunden/struktur`) read-only aus Dok. 14/16, **vollständig
preisfrei**. Sechs unabhängige Gates (2026-07-24): **4× FREIGABE** (Code, Product, Domain, Security),
**2× FREIGABE_MIT_AUFLAGEN** (QA, Konzepttreue); alle Auflagen im Fix-Pass behoben. O-WP006-01…07
stammen aus dem WP-Zuschnitt, 08/09 wurden beim Bauen gefunden.

| ID | Frage | Art | Aktueller Umgang | Owner / Gate |
|---|---|---|---|---|
| O-WP006-01 | **Zwei konkurrierende Demo-Firmenlisten:** Dok. 03 „Demo-, Test- und Beispielrollen" nennt **vier** (Nordwerk, Finovia, MediCore, Consulting Operator — heutiger Seed), Dok. 16 „Synthetische Demo-Daten und Demo-Dramaturgie" **fünf andere** (Nordstern, AlpenCloud, Rheinbank, MediNova, GreenGrid); DR-0009 spricht für WP-021 von „den fünf Konzept-Unternehmen" | **Konzeptspannung** | Stufe 1 bleibt auf dem heutigen Seed (Dok.-03-Namen); keine Umbenennung, keine Vermischung | Concept Author + Owner **vor dem WP-021-Schnitt** |
| O-WP006-02 | Service Family, Service Offer und Package haben **keinen kanonischen Objekttyp** (Dok. 07 F01–F09); der Katalog lebt als Code-Konstante statt als Daten | Konzeptlücke (vgl. O-WP012-01) | Katalog als quellenbelegte Konstante; kein Mapping Instanz↔Offer | Concept Author |
| O-WP006-03 | 06-D02 „Kundennutzer starten im eigenen Customer Workspace" — Stufe 1 löst das als **sichtbaren Einstieg** ohne Redirect (neutraler Einstieg DR-0009 bleibt) | Produktfrage | Einstieg gebaut, Redirect-Frage offen; schließt O-WP020-08 teilweise | Product / Owner |
| O-WP006-04 | **Kundensphäre ist Perspektive, keine technische Grenze:** eine Kundenrolle kann per Rollenwechsel weiter Portfolio-Sichten öffnen | Security-Grenze (bekannt, Demo) | neue Seiten strikt sphärenrein (**Security-Gate bestätigt**: Mandanten- und Sphärengrenze eingehalten); produktweite Erzwingung benannt statt vorgetäuscht | Security / Auth-WP (mit FINDING-0004/WP-030) |
| O-WP006-05 | Dok. 14 „Paketarchitektur": jedes Paket enthält als Pflichtbestandteil „eine illustrative Preisbandbreite" — kollidiert mit dem strengen Preis-Guardrail | Regel-Anpassung (präzisiert O-KUNDE-01) | Preisstellen als benannte Lücke; keine Guardrail-Änderung; Preisfreiheit negativ belegt (Test gegen Währung **und** währungslose Bänder) | Owner + Security beim WP-021-/Stufe-2-Schnitt |
| O-WP006-06 | Dok. 16 „Onboarding Workspace und rollenbezogene Sichten" (fünf Sichten) + Dok. 06 „Customer Workspace" (drei Lenses) — Stufe 1 baut eine gemeinsame Read-Struktur ohne Sichten-/Lens-Umschaltung | Lücke (spätere Stufe) | als benannte Lücke im Produkt | Product / UX (Stufe-2+-Schnitt) |
| O-WP006-07 | Die S02-Leitfrage „Wie verstehe ich diesen Kunden in einer Minute?" ist aus Berater-Perspektive; für Kundenrollen ist „dieser Kunde" das eigene Unternehmen | Wortlaut-Frage | sinngemäße, im Code begründete Formulierung; kein PDF-Zitat, das keines ist | Product + Concept Author |
| **O-WP006-08** | **Regelevolution:** „Exit Acceptance" (worttreuer Exit-Gate-Wert der Lifecycle-Phase 10, Dok. 16 „Lifecycle-Modell") kollidiert mit dem `prozessvokabular`-Wächter (`/\bAcceptance\b/`), ist aber Konzept-Fachbegriff, kein Projekt-Prozessvokabular; Umformulieren wäre stille Konzeptänderung | Wächter-Regelevolution | **enge, quellenbelegte Ausnahme**: maskiert nur die exakte Zwei-Wort-Wendung, Negativbeweis („Acceptance"/„Slice" bleiben verboten), Meta-Test an `LIFECYCLE_PHASEN[10].exitGate` gebunden; **von allen sechs Gates bestätigt** (Muster O-WP020-12). Beleg: `prozessvokabular.test.tsx` | QA + Product (**bestätigt**) |
| **O-WP006-09** | **`produktsprache`-Kollision (DR-0011 vs. Regel Null):** der Struktur-Assistent rendert „synthetischer Account, Demo-Story" (Dok. 16 Lifecycle-Phase 0 „Prospect/Sandbox") und „…Simulation…" (Phase 5 „Validate") — **worttreue** Dok.-16-Produktinhalte, die aber die von DR-0011 aus dem UI verbannten Wörter enthalten. Dieselbe Klasse wie O-WP032-11 (PDF-Produktvokabular, kein Disclaimer über unsere Daten) | Konzept-/Owner-Konflikt | **Interim:** worttreu belassen unter enger, mechanisch an `LIFECYCLE_PHASEN.ergebnisse` gebundener `produktsprache`-Ausnahme (Negativbeweis + Meta-Test, Muster O-WP032-11); die Seite steht damit **unter** dem Wächter, nicht daneben. **Owner entscheidet, ob DR-0011 eine Umformulierung erzwingt** | Product / Owner + Security |
| O-WP006-10 | **OL07 ↔ 16-D07-Delta „kommerzielle Baseline":** Das Verfassungsprinzip OL07 (Dok. 16 „Onboarding- und Lifecycle-Verfassung") nennt **sechs** freigabepflichtige Gegenstände (Scope, Zielprofil, Risikotoleranz, Serviceübernahme, **kommerzielle Baseline**, Go-live), die bindende Entscheidung 16-D07 nur **fünf** (ohne „kommerzielle Baseline") — am PDF verifiziert | **Intra-Dokument-Divergenz** | Stufe 1 folgt 16-D07 (bindende Entscheidung; der Slice ist preisfrei, DR-0008); das Delta ist im Code benannt statt still aufgelöst (`struktur.ts`) | Concept Author / Owner: „kommerzielle Baseline" bei der Preis-/Stufe-2-Erweiterung als sechste Position aufnehmen? |

## WP-025 – Cockpit (modernes Dashboard, DR-0014)

O-WP025-01..07 sind in `docs/project/design/WP-025_COCKPIT_VARIANTEN_KONZEPT.md` hergeleitet
(Personalisierungs-Granularität, adaptiv vs. getrennt, Ampel-Schwellen, Management-Modus,
Variantenwahl, Leitfrage-Wortlaut, Customer-Workspace-Kopf). Beim Bau des modernen Cockpits neu:

| ID | Frage | Art | Aktueller Umgang | Owner / Gate |
|---|---|---|---|---|
| O-WP025-06 | Der heutige Seed trägt **keine Control-/Risiko-Deckungslücke** (Nordwerk deckt sein einziges Control und Risiko ab) — die amber/rot-**Warnkarten** mit Objekt-360-Drilldown erscheinen erst, sobald ein Mandant eine ungedeckte Control/Risiko trägt | Daten-/Demo-Lücke | Pfad gebaut **und** unit-getestet (`objectDetailHref`-Mapping, Render-Test mit synthetischer Warnung); amber/rot heute live über die Deckungsringe (Owner/Vertrauensgrad) | **löst sich mit WP-021** (reiche Dok.-16-Demo-Welt + synthetische Bewertungen, DR-0015) |
| O-WP025-07 | Keine dedizierte **gefilterte Teillisten-Route** („nur die Lückenobjekte") — als präzises Ziel dienen die Objekt-360-Links je Lückenobjekt in der Warnkarte | Produktfrage (U-09-nah) | Objekt-360-Links je Lückenobjekt (funktional, kein toter Link) | Product / UX (spätere Filterroute) |
