...
- ein Presentation Repository mit Case-Manifests, Update Requests und geschützten manuellen Inhalten,
- regelmäßige Aktualisierung bestehender Decks statt unnötiger Neuerstellung,
- audience-gesteuerte Creative- und dezente Humor-Modi mit Rechte- und Freigabeprüfung.
...
### 10.5 Presentation Repository

PowerPoint-Cases werden nicht als lose Download-Dateien behandelt. Das Repository enthält eine eigene, versionierte Präsentationsstruktur:

- `presentations/templates/` für freigegebene Case- und Audience-Templates,
- `presentations/manifests/` für maschinenlesbare Presentation Manifests,
- `presentations/generated/` für reproduzierbare Demo- und Testartefakte,
- `presentations/snapshots/` für referenzierte Datenstände,
- `presentations/assets/` für freigegebene Diagramme, Icons, Illustrationen und Creative Visuals,
- `presentations/update-requests/` für geplante oder ereignisgesteuerte Aktualisierungen,
- `presentations/archive/` für ersetzte, weiterhin nachvollziehbare Fassungen,
- `presentations/tests/` für Rendering-, Content-, Accessibility- und Regressionstests.
...
Jedes regelmäßig gepflegte Deck besitzt ein maschinenlesbares Manifest. Pflichtfelder sind:

| Feld | Bedeutung |
|---|---|
| Presentation-ID | stabile Identität des Cases, unabhängig vom Dateinamen |
| Case Type | Vorstand, CISO, Management Review, Audit, Service Review, Workshop, Incident oder Angebot |
| Audience | primäre Rolle, Wissensniveau, Vertraulichkeit und erwartete Handlung |
| Mandant und Scope | Unternehmen, Organisationseinheit, Service, Audit oder Programm |
| Snapshot Policy | Datenstand, Zeitraum, Methoden- und Objektversionen |
| Template Version | verwendetes Layout, Branding, Pflichtfolien und Design Tokens |
| Sections | aktive Storyline-Blöcke und deren Reihenfolge |
| Protected Regions | manuell gepflegte Inhalte, die bei Updates erhalten bleiben |
| Refresh Rules | welche Datenblöcke automatisch aktualisiert, neu berechnet oder zur Review markiert werden |
| Review Policy | erforderliche Fach-, Zahlen-, Security-, Brand- und Freigaberollen |
| Audience Mode | formal, executive, operational, workshop oder creative |
| Humor Mode | off, subtle oder workshop-only; niemals implizit aktiviert |
| Distribution | zulässige Empfänger, Kanäle, Ablauf und Wasserzeichen |
| Status | draft, review, approved, published, superseded oder archived |

### 10.7 Kanonischer Case-Katalog
...
3. berechnet geänderte Claims, Charts, Tabellen und Empfehlungen,
4. erstellt einen semantischen Update-Diff,
5. aktualisiert nur freigegebene dynamische Bereiche,
6. erhält geschützte Einleitungen, Kommentare, Sprecherhinweise und kundenspezifische Aussagen,
7. markiert Konflikte zwischen alten manuellen Texten und neuen Daten,
8. fordert die erforderlichen Reviews an,
9. veröffentlicht nach Freigabe eine neue Version und archiviert die ersetzte Fassung.
...
### 10.10 Audience-, Creative- und Humor-Modi

Der Präsentationsstil wird bewusst gesteuert:

| Modus | Einsatz | Regeln |
|---|---|---|
| Formal | Audit, regulatorische oder externe Unterlagen | kein Humor; maximale Quellen- und Methodentiefe |
| Executive | Vorstand und Geschäftsführung | knapp, entscheidungsorientiert, seriös |
| Operational | CISO, ISMS und Servicebetrieb | handlungs- und detailorientiert |
...
Ein Presentation Update Request wird erzeugt, wenn sich mindestens einer der folgenden Punkte relevant ändert:

- KPI-Definition oder KPI-Wert,
- Risiko-, Control-, Threat- oder Zielstatus,
- aktive Empfehlung oder Decision Card,
- Serviceumfang, SLA, Preisannahme oder Value Ledger,
- Template, Branding, Disclaimer oder Zielgruppe,
- regulatorische Pflichtaussage,
- Produktfunktion oder UI, die im Deck gezeigt wird,
- freigegebene Research- oder Wettbewerbsbehauptung.

Der Request enthält Ursache, betroffene Decks, Priorität, erforderliche Reviewer, Update-Diff und spätesten Aktualisierungstermin. Der Presentation Curator aus Dokument 21 koordiniert die Pflege; die Veröffentlichung bleibt an menschliche Freigaben gebunden.

### 10.12 Presentation-as-Code und Binärartefakte

Templates, Manifests, Storylines, Datenverträge und Tests werden textuell versioniert. Erzeugte PPTX- und PDF-Dateien sind abgeleitete Artefakte. Für jeden Build werden mindestens festgehalten:

- Commit oder Build-ID,
- Manifest- und Template-Version,
- Snapshot-ID,
- Rendering-Engine und Version,
- Test- und Reviewstatus,
- Hash der erzeugten Datei,
- Freigabe- und Veröffentlichungsstatus.

Damit kann Claude Code Präsentationen reproduzierbar erzeugen und in späteren Sessions aktualisieren, ohne die gesamte Produktbibliothek neu verstehen zu müssen.
...
### 13.4 Template- und Case-Trennung

Ein Template definiert Layout, Tokens, Pflichtpositionen und visuelle Regeln. Ein Case definiert Kommunikationsziel, Storyline, Audience, Snapshot Policy und Update-Regeln. Dadurch kann derselbe Case in neutralem Plattformdesign, Betreiberbranding oder Mandantenbranding gerendert werden, ohne fachliche Inhalte zu duplizieren. Ebenso kann ein Theme-Update mehrere Cases kontrolliert erneuern, ohne deren Storyline still zu verändern.
...
### 25.7 Regelmäßiges Deck-Update mit geschützter Einleitung

1. Ein CISO-Monatsdeck besitzt eine manuell verfasste Einleitung und freigegebene Sprecherhinweise.
...
11. Repository-Browser zeigt Case-Manifest, Template-Version, Snapshot und Buildstatus.
...
- Jeder wiederkehrende Presentation Case besitzt ein versioniertes Manifest.
- Bestehende Decks können kontrolliert aktualisiert werden, ohne geschützte manuelle Inhalte still zu überschreiben.
- Ein semantischer Update-Diff zeigt geänderte Claims, Datenblöcke, Folien und Reviewbedarf.
- Templates und Cases sind getrennt versioniert und können unabhängig aktualisiert werden.
- Audience-, Creative- und Humor-Modi sind explizit, rollen- und freigabegesteuert.
- Rechteungeklärte Meme- oder Fremdvisuals werden nicht in freigegebene Artefakte übernommen.
- Presentation Update Requests können durch Daten-, KPI-, Research-, Template- oder Produktänderungen ausgelöst werden.
- Erzeugte Binärartefakte sind auf Manifest, Snapshot, Template, Build und Hash zurückführbar.
...
- **ENTSCHEIDUNG 12-16:** Wiederkehrende PowerPoint-Cases werden als versionierte Repository-Artefakte mit Presentation Manifest gepflegt.
- **ENTSCHEIDUNG 12-17:** Die Standardaktion ist ein kontrolliertes Update bestehender Decks, nicht immer eine vollständige Neuerstellung.
- **ENTSCHEIDUNG 12-18:** Manuelle Inhalte werden als locked, review-on-change oder replaceable klassifiziert und nie still überschrieben.
- **ENTSCHEIDUNG 12-19:** Templates, Cases, Snapshots und Binärartefakte besitzen getrennte Versionen und nachvollziehbare Buildbeziehungen.
- **ENTSCHEIDUNG 12-20:** Audience-, Creative- und Humor-Modi sind explizite, freigabegesteuerte Manifestparameter.
- **ENTSCHEIDUNG 12-21:** Meme-ähnliche Visuals sind nur als eigene, generierte oder eindeutig lizenzierte Assets zulässig und standardmäßig deaktiviert.
- **ENTSCHEIDUNG 12-22:** Der Presentation Curator aus Dokument 21 koordiniert Update Requests; externe Veröffentlichung bleibt menschlich freigabepflichtig.
...
- **ANNAHME 12-A11:** Presentation Manifests, geschützte Regionen und semantische Update-Diffs sind mit kontrollierten Templates technisch realisierbar.
- **ANNAHME 12-A12:** Nutzer bevorzugen bei wiederkehrenden Terminen eine Aktualisierung des vertrauten Decks gegenüber einer vollständig neuen Storyline.
- **ANNAHME 12-A13:** Für den Demonstrator genügen eigene neutrale Creative Assets; externe Meme-Bibliotheken sind nicht erforderlich.
- **ANNAHME 12-A14:** Die textuellen Presentation-as-Code-Artefakte können durch Claude Code über mehrere Sessions zuverlässig gepflegt werden.
...
- **OFFENE FRAGE 12-Q16:** Welche Presentation-as-Code- und PPTX-Bibliothek wird für editierbare Formen, Charts und Notes gewählt?
- **OFFENE FRAGE 12-Q17:** Welche manuellen Inhaltsarten können im ersten Release zuverlässig als geschützte Regionen erhalten werden?
- **OFFENE FRAGE 12-Q18:** Wie detailliert muss der Update-Diff auf Folien-, Block-, Claim- und Zeichenebene sein?
- **OFFENE FRAGE 12-Q19:** Werden erzeugte Binärartefakte im Git-Repository, über Git LFS oder ausschließlich in Object Storage abgelegt?
- **OFFENE FRAGE 12-Q20:** Wer darf Creative- und Humor-Modi für welche Audience und welchen Verteilungskanal freigeben?
- **OFFENE FRAGE 12-Q21:** Welche rechtliche und Brand-Prüfung ist für generierte Illustrationen oder Meme-ähnliche Visuals erforderlich?
...
- Live-Synchronisierung ausgewählter Charts mit Presenter View, ohne den freigegebenen Snapshot zu verändern,
- automatischer Presentation Health Score für Aktualität, Quellen, Storyline und Überladung,
- Creative Asset Generator mit Rechte-, Audience- und Brand-Guardrails,
- kuratierte interne Humorbibliothek mit expliziter Freigabe und Ablaufdatum,
- automatische Erkennung, ob ein Deck aktualisiert, neu erstellt oder bewusst unverändert gelassen werden sollte.
...
|---|---|---|---|
| 1.1 | 22.07.2026 | Presentation Repository, Case-Manifests, Update statt Neuerstellung, geschützte manuelle Inhalte, Update-Diff, Presentation Update Requests, Audience-/Creative-/Humor-Modi und Anbindung an Dokument 21 ergänzt | Aktiv |
| 1.0 | 21.07.2026 | Erstfassung der Reporting-, PDF-, PowerPoint-, Content-Block-, Storyline-, Template-, Snapshot-, Review-, Freigabe-, Verteilungs- und Outcome-Architektur | Archiviert |
